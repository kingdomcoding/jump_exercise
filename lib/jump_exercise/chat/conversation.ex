defmodule JumpExercise.Chat.Conversation do
  use Ash.Resource,
  ...
  relationships do
    has_many :messages, JumpExercise.Chat.Message do
      public?(true)
    end
  ...
  actions do
    ...
    update :generate_name do
      ...
      change(JumpExercise.Chat.Conversation.Changes.GenerateName)
    end
  ...
  calculations do
    calculate :needs_title, :boolean do
      calculation(
        expr(
          is_nil(title) and
            (count(messages) > 3 or (count(messages) > 1 and inserted_at < ago(10, :minute)))
        )
      )
    end
  end
