defmodule JumpExercise.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%JumpExercise.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(JumpExercise.Accounts.User, id, authorize?: false) do
